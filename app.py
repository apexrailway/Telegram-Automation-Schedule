...
class WebTelegramForwarder:
    ...
    def finish_connection_process(self, connected_count, failed_count):
        ...
        # Auto-start health check if we have connections
        if connected_count > 0 and not self.health_check_running:
            self.health_check_running = True
            if self.loop:
                asyncio.run_coroutine_threadsafe(self.health_check_loop(), self.loop)
    ...
    async def ensure_connected(self, phone):
        """Ensure a client is connected. Reconnect if disconnected."""
        if phone not in self.clients:
            return False

        client = self.clients[phone]
        
        # is_connected() is a synchronous check of the connection state
        if client.is_connected():
            # Check if user is authorized, just in case
            try:
                if await client.is_user_authorized():
                    return True
            except Exception as e:
                self.log_message(f"Auth check error for {phone}: {e}")
        
        self.log_message(f"⚠ Client {phone} disconnected. Attempting to reconnect...")
        return await self.reconnect_client(phone)

    async def reconnect_client(self, phone):
        ...
        try:
            ...
            if not account or not account.session_string:
                self.log_message(f"✗ Cannot reconnect {phone}: No session string found")
        ...
        try:
            ...
            self.log_message(f"Reconnecting {phone} using saved session...")
            
            client = TelegramClient(
                StringSession(session_string),
                int(account_dict['api_id']),
                account_dict['api_hash'],
                timeout=20,
                retry_delay=1,
                auto_reconnect=True,
            ...
            if await client.is_user_authorized():
                ...
                self.log_message(f"✓ Successfully reconnected {phone}")
                # Save potentially updated session
                await self.save_session_to_db(phone, client.session.save())
                return True
            else:
                self.log_message(f"✗ Reconnect failed for {phone}: Auth expired")
                return False
                
        except Exception as e:
            self.log_message(f"✗ Reconnect error for {phone}: {e}")
            return False

    async def health_check_loop(self):
        """Periodically check all connected clients and reconnect if necessary"""
        self.log_message("🏥 Health check loop started")
        while self.health_check_running:
            try:
                phones_to_check = list(self.clients.keys())
                for phone in phones_to_check:
                    if not self.health_check_running:
                        break
                    await self.ensure_connected(phone)
                    await asyncio.sleep(2) # Stagger checks
            except Exception as e:
                self.log_message(f"Health check loop error: {e}")
        ...
        self.log_message("🏥 Health check loop stopped")
    ...
    async def send_scheduled_post(self, post):
        ...
        try:
            ...
            for phone, channels in post['channels'].items():
                ...
                is_connected = await self.ensure_connected(phone)
                if not is_connected:
                    self.log_message(f"⚠ Could not ensure connection for {phone} - skipping channels")
                    for channel in channels:
                        total_count += 1
                        failed_details.append(f"Channel {channel}: Account {phone} disconnected")
                    continue
                
                # Update client reference in case it was recreated during reconnect
                client = self.clients[phone]
                ...
                for channel in channels:
                    ...
                    while retry_count < max_retries and not sent:
                        try:
                            ...
                            retry_count += 1
                            if retry_count < max_retries:
                                self.log_message(f"⚠ Connection drop detected while sending to {channel}. Attempting to reconnect...")
                                is_connected = await self.ensure_connected(phone)
                                if is_connected:
                                    client = self.clients[phone]
                                    self.log_message(f"✓ Reconnected successfully, will retry send...")
                                else:
                                    self.log_message(f"✗ Could not reconnect for channel {channel}")
                                await asyncio.sleep(2)
                            else:
                                failed_details.append(f"Channel {channel} ({phone}): ConnectionError")
                                self.log_message(f"✗ Failed to send to channel {channel} after {max_retries} attempts: ConnectionError")
    ...
    async def send_single_scheduled_post(self, client, post_input, target_channel, phone):
        try:
            ...
            # Connection dropped during send - raise so it can be retried
            raise
